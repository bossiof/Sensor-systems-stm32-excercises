...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
	  //setting of the right column and resetting of the others
	  for(column_index=0;column_index<4;column_index++){
		  ...
		  //delay to avoid errors for multiple button presses
		  HAL_Delay(100);
	  ...
	  //now we have an array where 1's are placed where the button is pressed
	  //we scan it and send out to the UART the corresponding character
	  for(int i=0;i<16;i++)
		  if(keypress[i]==1){
			  if(ack[i]==0){
				  //if we have yet to acknowledge this event that means is a new valid keypress
				  //we get the character to send from the map array
				  //at the position corresponding to the keypress
				  HAL_UART_Transmit(&huart2, &map[i], 1, 10);
				  //then we ack this keypresss so later we ignore it if it remains pressed
				 ack[i]=1;
			  }
			  else{
				  //the key is releaserd-> remove the ack
				  ack[i]=0;
			  }
		  }
    /* USER CODE BEGIN 3 */
